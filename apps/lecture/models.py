...
class LectureMarker(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="lecture_markers"
    )
    lecture = models.ForeignKey(
        Lecture, on_delete=models.CASCADE, related_name="markers"
    )
    timestamp = models.FloatField(help_text="Marker timestamp in seconds")
