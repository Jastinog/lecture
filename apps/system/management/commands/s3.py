...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"   USE_S3_MEDIA: {getattr(settings, 'USE_S3_MEDIA', 'NOT_SET')}"
        )
        self.stdout.write(
            f"   AWS_STORAGE_BUCKET_NAME: {getattr(settings, 'AWS_STORAGE_BUCKET_NAME', 'NOT_SET')}"
        )
        self.stdout.write(
            f"   AWS_S3_REGION_NAME: {getattr(settings, 'AWS_S3_REGION_NAME', 'NOT_SET')}"
        )
        ...
        self.stdout.write(
            f"   default_storage._wrapped type: {type(default_storage._wrapped)}"
        )
        ...
        self.stdout.write(
            f"   Has bucket_name: {hasattr(storage_instance, 'bucket_name')}"
        )
        ...
        try:
            ...
            self.stdout.write("   ✓ Direct S3 upload successful")

            # Clean up
            s3_client.delete_object(
                Bucket=settings.AWS_STORAGE_BUCKET_NAME, Key=test_key
            )
            self.stdout.write("   ✓ Direct S3 test file cleaned up")
